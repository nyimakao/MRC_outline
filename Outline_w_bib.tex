...
\begin{defn}
The foliations $\W^{cs}$ and $\W^u$ have \textit{local product structure} at scale $\delta>0$ with constant $\kappa$ if \color{blue}{bracket operation is well-defined}.
\end{defn}
...
We begin by fixing $(v_{0},t_{0})\in C_{T}(\eta)$, $\alpha>1$, and $\epsilon>0$
